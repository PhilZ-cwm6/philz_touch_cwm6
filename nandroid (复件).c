...
#else
    sprintf(tmp, "cd $(dirname %s) ; touch %s.tar ; twrptar -c %s -d %s -t %s.tar", backup_path, backup_file_image, strcmp(backup_path, "/data") == 0 && is_data_media() ? "-m" : "", backup_path, backup_file_image);
#endif
...
#else
    sprintf(tmp, "cd $(dirname %s) ; touch %s.tar.gz ; twrptar -c -z %s -d %s -t %s.tar.gz", backup_path, backup_file_image, strcmp(backup_path, "/data") == 0 && is_data_media() ? "-m" : "", backup_path, backup_file_image);
#endif
